...
class PolicyEngine:
    ...
    def validate_action(
        ...
        for forbidden in policy.forbidden_actions:
            if forbidden.name == action_name:
                return {"status": "blocked", "reason": f"Forbidden: {forbidden.reason}"}
        ...
        if allowed_action is None:
            return {"status": "blocked", "reason": f"Action '{action_name}' not in allowed list"}

        if environment == "production":
            return {"status": "requires_approval", "reason": "Production environment requires approval"}

        if allowed_namespaces and namespace not in allowed_namespaces:
            return {"status": "blocked", "reason": f"Scope violation: namespace '{namespace}' not allowed"}

        if allowed_action.blast_radius in ("medium", "high"):
            return {"status": "requires_approval", "reason": f"Blast radius '{allowed_action.blast_radius}' requires approval"}

        if not allowed_action.rollback_path:
            return {"status": "requires_approval", "reason": "No rollback path defined"}
    ...
    def validate_plan(
        ...
        return [
            self.validate_action(name, policy, namespace, environment, allowed_namespaces)
